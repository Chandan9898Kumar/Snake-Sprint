...
class App extends Component {
  ...
  updateSnakeState() {
    ...
    while (currentBodyPiece) {
      //  This below code is having some warnings : Function declared in a loop contains unsafe references to variable(s) 'currentBodyPiece', 'currentBodyPiece'.

      //                  Solution:
      // The warning you're encountering about "unsafe references" typically arises when a variable is captured in a closure within a loop,
      // which can lead to unexpected behavior if the variable changes during the loop's execution. In your case, currentBodyPiece is being referenced inside the some
      // and filter methods, which could lead to issues if currentBodyPiece is modified in subsequent iterations of the loop.
      ...
      // To resolve this, you can create a copy of currentBodyPiece or use a different approach to ensure that the reference remains stable throughout
      // the execution of the loop. Here’s a refactored version of your code that avoids unsafe references:

      /**
       * Changes Made:
1. Storing Coordinates: I stored currentBodyPiece.coordinates in a separate variable currentCoordinates to avoid referencing currentBodyPiece directly in the loop.
2. Using find Instead of filter: I replaced the filter method with find, which is more appropriate when you only need the first matching element. This avoids creating an array unnecessarily.
3. Check for Existence: Added a check to ensure that turningPoint exists before trying to access its properties.
       */
      const currentCoordinates = currentBodyPiece.coordinates;

      if (
        this.turningPoints.some(
          (x) =>
            x.coordinates[0] === currentCoordinates[0] &&
            x.coordinates[1] === currentCoordinates[1]
        )
      ) {
        const turningPoint = this.turningPoints.find(
          (x) =>
            x.coordinates[0] === currentCoordinates[0] &&
            x.coordinates[1] === currentCoordinates[1]
        );
        ...
        if (turningPoint) {
          currentBodyPiece.direction = turningPoint.nextDirection;
    ...
    if (this.allowSwipe) {
      if (Math.abs(x) > tolerance || Math.abs(y) > tolerance) {
        ...
        if (Math.abs(y) > Math.abs(x))
          this.keyListener({ keyCode: y > 0 ? 40 : 38 });
        else this.keyListener({ keyCode: x > 0 ? 39 : 37 });
      }
    }
  }
